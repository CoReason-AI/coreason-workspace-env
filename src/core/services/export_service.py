...
class PlatformExporter:
    ...
    async def bundle_agent_specs(self, session_id: str, tenant_id: str = "default_tenant") -> Optional[str]:
        ...
        if not os.path.exists(session_dir):
            logger.warning(f"No generated files found on disk for session {session_id}. Querying Postgres state.")
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    ...
                    if records:
                        ...
                        raise ValueError(f"No finalized state found for session {session_id}")
...
name = "coreason-agent-{session_id}"
version = "0.1.0"
...
"""
        ...
        try:
            ...
            logger.info(f"Successfully bundled platform for {session_id} at {zip_path}")
            return zip_path
        except Exception as e:

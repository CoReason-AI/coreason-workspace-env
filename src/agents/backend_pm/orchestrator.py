...
class BackendPmAgent(DeepAgent):
    """
    Project Manager for orchestrating the Backend Maker-Checker pipeline.
    """
    ...
    def _run_maker(self, state: MakerCheckerState):
        from src.agents.fastapi_coder.orchestrator import FastApiCoderAgent
        worker = FastApiCoderAgent()
        context = state.get("messages", [])
        feedback = state.get("feedback", "")
        worker_context = f"{context}\nFeedback from previous run: {feedback}" if feedback else str(context)
        
        attempts = state.get("attempts", 0) + 1
        result = worker.execute(worker_context, "backend_pm_loop")
    ...
    def _run_validator(self, state: MakerCheckerState):
        ...
        validation = validator.execute(state["worker_result"], "backend_pm_loop")
    ...
    def execute(self, state: dict, session_id: str = None) -> str:
        ...
        """
        logger.info(f"[{session_id}] BackendPM initiating LangGraph StateGraph pipeline.")
        initial_state = {
